...
| F-31 | The `lockorder` job aborts every script that has a peer: Ycash v4.5.0's own `getpeerinfo` vs `SendMessages` lock order trips `sync.cpp`'s assertion before any Yellowback code runs (reproduced locally; the four sites are byte-identical to the pin). `reindex.py`, with no peers, is the one pass | **Owner decision needed.** (a) a debug-only `DEBUG_LOCKORDER_LOGONLY` switch in `sync.cpp` that logs inversions instead of aborting, with the job failing on any inversion that names `cs_yellowback` or a `yellowback/` site — the only way the job yields evidence about the fork's own locks; (b) drop the job; (c) leave it red. Nothing changed yet (2026-09-23) |
