...
| F-27 | The CI nightly's "Devnet role presets" step died in every preset at `quote agents did not publish within 60s`: the nightly and main jobs ran ubuntu-22.04's Python 3.10, and the quote agent the devnet starts imports the stdlib `tomllib` (3.11+), so each agent crashed on import before its first quote. The same gap had kept `yellowback_quote` out of CI since Phase 7 (recorded in the workflow's `env` comment, never closed) | Both jobs take Python 3.11 from `actions/setup-python` (not 3.12: the inherited framework still imports `asyncore`) with `pyzmq`/`simplejson` from pip; `yellowback_quote` joins the merge gate's list; the prerequisite check imports `tomllib` (2026-09-22) |
| F-28 | The nightly's `make check` failed in the inherited `bitcoin-util-test.py`: Ycash v4.5.0 renamed `zcash-tx` to `ycash-tx` without updating the test vectors, and ten of the cases also expect Zcash `t1…` addresses where Ycash prints `s1…`. Reproduced at the pin; both files are frozen | CI runs the other three parts of `make check` by hand (`check-TESTS`, secp256k1, univalue); recorded in `doc/yellowback.md`'s baseline section (2026-09-22) |
| F-29 | Three more nightly variants never got past configure: lockorder (`sync.cpp` used `std::set`/`std::map` without including them under `DEBUG_LOCKORDER`, which no other build compiles), sanitizers and coverage (`TMPDIR` created after `./configure`, and `config.guess` needs it to exist), and both fuzz jobs (`-fsanitize=fuzzer` in `CXXFLAGS` links libFuzzer's `main()` into configure's own "C++ compiler works" program, "cannot create executables") | `sync.cpp` includes the three headers; `mkdir -p "$TMPDIR"` precedes configure; fuzz configures with `fuzzer-no-link` and adds `-fsanitize=fuzzer` on the `ycashd` link alone (`ycashd_LDFLAGS` override), dumping `config.log` on failure; `doc/yellowback.md`'s fuzz recipe updated (2026-09-22). The variant jobs run on schedule or "Run workflow", so these fixes are verified only at the next nightly |
| F-30 | The nightly's "Reorg stress" step failed at `yed_getinfo['synced']`: `yellowback_reorg_stress.py` is still the v1 federation script (rosters, genesis anchors, cosigned price transactions) and its v2 adaptation is the unchecked Phase 6 item of the v2 plan. With the `synced` helper fixed it dies at the next v1 field (`anchor`) | Step removed from the nightly with the reason in the workflow; the two v1 helpers now read v2's `healthy`/`height`. **Open item for the owner:** a v3 reorg-stress rewrite (random tags, attestations, enforcement events over a few hundred blocks, state hash equal on every node, after restart and cold rebuild) is a chunk of its own; reorgs are covered today by ten other scripts (2026-09-23) |
| F-31 | The `lockorder` job aborts every script that has a peer: Ycash v4.5.0's own `getpeerinfo` vs `SendMessages` lock order trips `sync.cpp`'s assertion before any Yellowback code runs (reproduced locally; the four sites are byte-identical to the pin). `reindex.py`, with no peers, is the one pass | Owner decision (2026-09-23): the hardening path. A debug-only `DEBUG_LOCKORDER_LOGONLY` switch in `sync.cpp` (dead code outside the lockorder job) keeps the detector fully armed and logging while the process no longer aborts; the job fails on any report naming `cs_yellowback`, a `yellowback/` site or a `yed_` RPC, and prints the distinct inherited sites for the record. The armed detector then found the two fork inversions and the inherited race of F-34/F-35 |
| F-34 | With the detector armed, seven scripts produced six distinct reports naming `cs_yellowback`: `YellowbackWallet::Reconcile` and the raw-transaction burn check took `cs_wallet` under `cs_yellowback`, and `yed_mint`, `yed_registerattestor`, `yed_signattestation`, the carrier send path and `yed_validaterawtransaction` held `cs_yellowback` while reaching `mempool.cs` (`FetchInputs`, `CommitTransaction` → `AcceptToMemoryPool`) — against the miner's `cs_main` > `mempool.cs` > `cs_yellowback`. Real deadlock candidates between an RPC and block template creation | The fork's own documented order (N25: `cs_main` > `cs_wallet` > `mempool.cs` > `cs_yellowback`) is now honoured everywhere: 25 RPC sites take `mempool.cs` before `cs_yellowback`, the two wallet sites take `cs_wallet` first, `Bonds`/`HotKeys` assert both. Verified: twelve scripts under the armed detector report only the inherited pair (2026-09-23) |
| F-35 | The debug build segfaulted in the Sapling-funded mint of `yellowback_attest_wallet`: Ycash v4.5.0's `AsyncRPCOperation_sendmany::find_utxos` calls `AvailableCoins` (which asserts `cs_wallet`) on the async worker with no lock held — a wallet-map data race in every stock `z_sendmany`, invisible in release builds, and a NULL dereference of the worker's lock stack under `DEBUG_LOCKORDER` | `find_utxos` takes `LOCK2(cs_main, cs_wallet)` (the fix later Zcash releases carry; the file is not frozen) and `sync.cpp`'s assertion helpers treat a NULL stack as nothing held. Recorded in `doc/yellowback.md`'s baseline (2026-09-23) |
| F-36 | With F-34 fixed, `yed_getinfo` showed as the next inversion: it read the wallet's locked-output count (`LockedCount`, `cs_wallet`) while holding `cs_yellowback` and `mempool.cs`, against every wallet RPC's `cs_wallet` > `mempool.cs` > `cs_yellowback` | The count is read under `cs_main` alone before the index locks. Verified under the armed detector (2026-09-23) |
| F-37 | The last Yellowback-naming pair is the miner's: `TemplateView` holds `cs_yellowback` for the whole of `CreateNewBlock`, including `TestBlockValidity`, which takes the script-check queue's `ControlMutex`; `ConnectTip` takes `ControlMutex` before `CheckConnect` takes `cs_yellowback`. Both paths hold `cs_main` from their first line, so they are mutually exclusive and the pair cannot deadlock | Allow-listed in `qa/yellowback-lockorder-check.py` with the proof; every other Yellowback-naming report fails the job. **Open item:** the structural fix is `ybview.reset()` before `TestBlockValidity` in `miner.cpp`, one line in a file frozen against `feature/yellowback-sf`; it goes in when miner.cpp is next open (2026-09-23) |
| F-38 | The merge gate failed once on `0737509fd` (a devnet-only commit) in `yellowback_attest.py`'s reorged-citation step: `reachable` was 0 where the test expected 1. Not a regression: the test assumed the attestors not fed on the reorged branch keep a fresh pre-split attestation, but that attestation cites a fixed height and the regtest window is 8 blocks, while the intersection height the test picks lies 1–4 blocks past the split, so at the far heights it is exactly stale. Which height is picked follows the block hashes, hence intermittent | The test computes the expectation from the actual heights (`citedHeight > R - ATTEST_MAX_AGE`) and asserts each selected seq's `poolFresh` accordingly; the `reachable < 2` branch already covered the low case. Three local runs green (2026-09-24) |
| F-39 | The merge gate failed on `6ec66eea0` in `yellowback_attest_wallet.py`: the mint's attest payee was not among the bond addresses the test expected. `register_and_arm` puts its raw registrations in one block, and the block's transaction order, not the fixed-key index, decides which key becomes seq 2, 3 or 4; the framework resolves the true mapping from `yed_listattestors`, but the wallet test computed the address as key `seq - 2` and the agent test (F-33) imported the hot key as `ATTESTOR_WIFS[seq]`. Both hold on most draws and fail on the rest | The framework gains `hot_wif_for(node, seq)` (the WIF behind the seq's registered pubkey); the agent test uses it, and the wallet test takes the bundle seqs' `bondKeyAddress` from the chain. Local runs: attest_wallet ×2, attest_agent ×2 (2026-09-24) |
| F-40 | A third intermittent in `yellowback_attest_wallet.py`, seen locally (1 of 2 runs): the Sapling-funded mint, one block after the funding note was mined, was refused with the builder's own "missing witness for a Sapling note; retry after the next block" — the wallet had not built the note's witness yet | The test follows the refusal's instruction: on exactly that error it mines one block and retries, at most three times, printing how many it took. Three local runs green (2026-09-24). Whether the witness lag itself is inherited wallet behaviour or a fork timing issue is not settled; the refusal is the designed answer either way |
| F-32 | `sanitizers` ("linker did not accept requested flags"), `coverage` ("Cannot enable RELRO", the first fatal link test after `--coverage`) and both fuzz jobs ("cannot create executables") all fail in configure for one reason: `config.site` replaces the command line's `CC=clang` with depends' clang 18 targeting `x86_64-pc-linux-gnu`, whose compiler-rt runtimes live under `x86_64-unknown-linux-gnu` and are not found | The variant steps link the host-triple runtime directory to the tarball's before configuring, print `-print-file-name=libclang_rt.asan.a` as evidence, and dump the relevant `config.log` section on failure. Unverified until the next dispatch run (2026-09-23) |
| F-33 | The nightly's "Attestor agent end to end" step, green on earlier nightlies, failed once with `poolFresh stayed at 2 (wanted 3) for 120s` on the first tick; the agents' own logs are not uploaded, so the slow signer left no evidence | The test now prints every agent's log tail when a wait stalls. **Root cause found 2026-09-24 (F-39):** the test imported node 7's hot key as `ATTESTOR_WIFS[seq]`, but the key index and the seq agree only when the block's transaction order matches the registration order, so on the other draws node 7 held a different seq's key and never signed |
