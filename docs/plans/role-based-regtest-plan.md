...
| F-21 | Same output: `Could not parse stylesheet of object MainWindow` and one `QObject::disconnect: Unexpected nullptr parameter` | The stylesheet is upstream YecWallet's theme file, unchanged in the fork (its loader and `res/` are byte-identical to v4.5.0); harmless, left alone. The disconnect warning is not reproduced by the 91-case QTest run, and the fork's only disconnect calls are guarded; it is most likely upstream start-up code. Both noted, neither blocks a walk-through |
