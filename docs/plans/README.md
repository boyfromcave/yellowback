...
| Document | What it is |
|---|---|
| [`ydollar-v1-development-plan.md`](ydollar-v1-development-plan.md) | **The plan.** Decision record, normative v1 protocol, code architecture in `ycash-dd`, federation coordinator, phased work plan with exit criteria, test plan, trust statement and threat model, and the later consensus-enshrinement path. Every `**OPEN**` item in `../spec/ydollar-adaptation-spec.md` is decided here. |
...
The plan's one-line summary: **YDollar v1 is a Tier-0 overlay** — colored transparent outputs
