...
linker silently dropped every Rust symbol and the build still succeeded); the frb loader uses
`ExternalLibrary.process` on iOS. Android needed no change. Device-driven test helpers
(`app/integration_test/device.dart`: scroll-aware taps, value-aware switches) replaced blind
finds. Runbook: README "Running on a simulator / emulator" (`scripts/run-ios.sh`,
`scripts/run-android.sh`, `--test m1|m2`).
