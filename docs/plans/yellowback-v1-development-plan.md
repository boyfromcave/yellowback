...
|---|---|---|---|
| A1 | Revision 1 put an `evhttp` client and an `AsyncRPCOperation` inside `ycashd` for redemption co-signing: outbound plaintext HTTP from the node, ≈ 250 new C++ lines, and a new failure surface in the wallet process. | design | **Removed.** `ycashd` does no outbound networking. `yed_redeem` returns the owner-signed hex; a Python client in `contrib/yellowback/` collects co-signatures over HTTPS; `yed_submitredeem` verifies and broadcasts. (D16) |
...
| A3 | XFER-2 burned *all* inputs when a TRANSFER under-assigned; DigiByte rejects such a tx at consensus, so its users lose nothing, whereas an overlay "reject" is a burn. | user-loss | Assignments with `Σ ≤ yedIn` are honoured and only the remainder burns; `Σ > yedIn` assigns nothing. (§3.7) |
| A4 | Transactions with two `OP_RETURN` outputs are non-standard but consensus-valid (a miner can include one); rule was unstated. | determinism | Explicit: more than one `OP_RETURN` ⇒ non-Yellowback. (§3.2) |
...
| A10 | Ycash transparent keys are a random keypool, not HD (`CWallet::GenerateNewKey`, HD seed is Sapling-only in 4.5). A vault owner key lives only in `wallet.dat`. | operational | One fresh key per mint for both token output and owner; documented backup rule; `yed_mint` warns when the keypool is low. (§4.5) |
...
| B4 | Coin locking ran only after a block was applied. A wallet's own 0-conf outputs are *trusted* and spendable by `sendtoaddress`/`z_sendmany` at once (`CWalletTx::IsTrusted`, `ref/ycash/src/wallet/wallet.cpp:4842-4867`), so a `sendtoaddress` issued right after `yed_mint`/`yed_send` could burn the fresh Yellowback. Yellowback received in a block is spendable between `SyncTransaction` and `ChainTip` of the same notifier cycle (`validationinterface.cpp:213-217`). | **user-loss** | Three-stage locking: (i) `yed_mint`/`yed_send`/`yed_redeem` lock their own YED outputs **before** `CommitTransaction`; (ii) the index's `SyncTransaction` override pre-locks every output a well-formed payload assigns to a script that is mine, for mempool and block transactions alike; (iii) `ChainTip` reconciles against the state. Over-locking a VOID mint's token output is undone at (iii). (§4.3, §4.5) |
...
| B12 | `Solver` classifies any `OP_RETURN` followed by *any* push-only sequence as `TX_NULL_DATA` (`ref/ycash/src/script/standard.cpp:102`); the payload shape was under-specified. | determinism | The payload is the single data push that follows `OP_RETURN`; any other shape is non-Yellowback. (§3.2) |
...
| B14 | The builder hand-set `nVersion`/`nVersionGroupId`/`nExpiryHeight` and ran a `GetMinimumFee` loop. Ycash already has `CreateNewContextualCMutableTransaction(consensus, nextHeight)` (`ref/ycash/src/main.cpp:7364-7392`), and every `z_*` operation uses a flat `DEFAULT_FEE = 1000` zat (`ref/ycash/src/policy/fees.h:15`). | simplification | Use both; smallest-first input selection as `AsyncRPCOperation_sendmany::find_utxos`; `-yellowbackfee` overrides. Builder budget ≈ 400 lines, not 900. (D9, §4.5) |
| B15 | Lock ordering was unstated. The notifier thread must not take `cs_main` in block callbacks (`validationinterface.cpp:162-166`); `LockCoin` requires `cs_wallet` (`wallet.cpp:6265`). | safety | Order `cs_main → cs_wallet → cs_yellowback`. The `ChainTip`/`SyncTransaction` handlers take `cs_yellowback` for the state update, release it, then take `cs_wallet` for locking; they never take `cs_main`. RPCs take `LOCK2(cs_main, cs_wallet)` then `cs_yellowback`. (§4.3) |
| B16 | Signers must agree on the sighash branch ID. `rpc/atomicswap.cpp:760` uses `Height()`, `signrawtransaction` uses `Height() + 1` (`rawtransaction.cpp:1024`). | precision | Every Yellowback signer uses `CurrentEpochBranchId(chainActive.Height() + 1)`; the two differ only at an upgrade boundary and none is scheduled (NU5 is `NO_ACTIVATION_HEIGHT`). (§3.3) |
...
| B18 | "Refuse when fewer than 2 keypool keys remain" would fail under the test framework's `-keypool=1`. | precision | `yed_mint` calls `TopUpKeyPool()` first (wallet must be unlocked) and refuses only if the pool is still short. (§4.5) |
| B19 | `yed_redeem` locks inputs but nothing unlocked them if the redemption was abandoned. | operational | `yed_abortredeem <hex>` unlocks the inputs of a redemption that was never broadcast. (§4.4) |
...
| B22 | Ycash builds with `-DYCASH_WR` (`zcutil/build.sh:81-85`) add `-deletetx`, which prunes old wallet transactions from `wallet.dat` (`wallet.cpp:4112-4400`). | compatibility | It keeps any transaction with an unspent transparent output that is mine (`wallet.cpp:4336-4348`), so YED coins survive; the index, not the wallet, is the source of `yed_listtransactions`, so history survives too. Documented, no code. (§4.5) |
| B23 | Operator key import with `importprivkey` triggers a full rescan. | operational | `importprivkey <key> "" false`. (§4.6) |
| B24 | The redemption client's deadline was implicit. | precision | With `nExpiryHeight = indexTip + 40` and `TX_EXPIRING_SOON_THRESHOLD = 3` (`main.h:81`), a redemption must be broadcast within 36 blocks (≈ 45 min) of `yed_redeem`; the client shows the deadline and `yed_abortredeem` recovers otherwise. (§5) |
...
rebuildable LevelDB, D10's address bytes (re-verified by exhaustive Base58 search, mainnet bytes updated in revision 13: `0x1FE4`→`ye`,
...
| C16 | `YELLOWBACK_FEE = DEFAULT_FEE` was presented as convention. ZIP-401's limiter adds `LOW_FEE_PENALTY` to the eviction weight of any transaction paying less than `DEFAULT_FEE` (`ref/ycash/src/mempool_limit.cpp:151-157`). | precision | `-yellowbackfee` is clamped to ≥ `DEFAULT_FEE`; the reason is recorded. (§3.1) |
| C17 | "Maximum output $100,000 as DigiByte (`digidollar.h:86-88`)": DigiByte defines no maximum output — those lines hold min mint, max mint and min output. | accuracy | `MAX_OUTPUT` is a Yellowback parameter equal to DigiByte's maximum mint (`digidollar.h:87`). (D11, §3.1) |
...
| C20 | A YED change amount in `(0, MIN_OUTPUT)` cannot be assigned (XFER-1) and would burn. | precision | `yed_send`/`yed_redeem` refuse such a change; users add or remove cents. (§4.5) |
...
| C22 | `CommitTransaction` adds the transaction to the wallet **before** `AcceptToMemoryPool`; a mempool rejection leaves a stale wallet record (`ref/ycash/src/wallet/wallet.cpp:5723-5747`). | precision | `yed_submitredeem` checks `chainActive.Height() ≥ lockHeight`, the expiry margin and `VerifyScript` before committing. (§3.8 SUB-1) |
...
| C25 | The atomic-swap precedent was quoted as "17 lines in `main.cpp`". It also changed ≈ 400 lines of `wallet.{h,cpp}` and `walletdb.{h,cpp}` (`ref/ycash` commit `ccddd22e4`). | accuracy | Yellowback's zero-lines target for `src/wallet/` is stricter than the precedent, not equal to it. (§1) |
...
| D2 | Shutdown was described as "mirrors ZMQ" (unregister at `init.cpp:255`). On the normal path `WaitForShutdown` interrupts and **joins** the thread group before `Shutdown()` (`ref/ycash/src/bitcoind.cpp:52-55`), so the notifier is gone; on the startup-failure path `join_all` is deliberately skipped (`bitcoind.cpp:191-194`) and `Shutdown()` can run while `ThreadNotifyWallets` is inside a `ChainTip` call. `UnregisterValidationInterface` does not wait for a running slot, so deleting the index there is a use-after-free on that path. | safety | The index is never deleted during `Shutdown()`. Teardown is: `UnregisterValidationInterface(g_yellowback)`; then `{ LOCK(cs_yellowback); g_yellowback->Stop(); g_yellowback->Flush(fSync = true); }` — taking `cs_yellowback` waits out any in-flight handler, `Stop()` makes every later handler return at once, and the object is left for process exit (as Ycash leaves `pcoinsTip`'s siblings). Placed right after `StopRPC()` so no RPC can race it. (§4.3) |
| D3 | `yed_redeem` "locked the consumed coins" and `yed_abortredeem` "unlocked" them. With C10 a REDEEM has no YEC inputs, and its YED inputs are *already* `LockCoin`-locked as tokens — unlocking them on abort would expose them. Nothing stopped a second `yed_redeem`/`yed_send` from re-using the same YED inputs while co-signatures were being collected, and SUB-1 had no defined place to find "the transaction `yed_redeem` produced". | precision | `src/yellowback/wallet.cpp` keeps an in-memory `PendingRedemption { vaultOutpoint, ownerSignedTx, reservedInputs, createdHeight }` map. `yed_redeem` records one (refusing if the vault already has one); YED input selection skips reserved outpoints and outpoints the wallet already reports spent by an unconfirmed transaction (`CWallet::IsSpent`, `ref/ycash/src/wallet/wallet.cpp:1087-1099`); `yed_submitredeem` compares against the record (SUB-1) and clears it on commit; `yed_abortredeem <vaultTxid>` clears it; records die with the process and expire with the transaction. `LockCoin` is not involved in a REDEEM at all. (§3.8, §4.4, §4.5) |
| D4 | `yed_createpricetx` needs the current anchor's value (new anchor = anchor + refill − fee; `prevtxs.amount`), but `Anchor` stored only `{outpoint, scriptPubKey}`. | precision | `Anchor` records `nValue` (from `vout[0].nValue` on every anchor move); the RPC also cross-checks it against the UTXO view under `cs_main`. (§3.5, §4.4) |
...
| D9 | Anyone can create VOID vaults (a MINT payload with a P2SH `vout[0]` and any failing rule) and every Yellowback node records them forever. | operational | Bounded by the transaction fee like any UTXO growth; `yed_listvaults` is paged; Phase 6 DoS review measures records-per-block at the mempool's cost limit. (§6) |
...
| E8 | Build placement was unstated; Ycash links `test_bitcoin` and `ycashd` from `libbitcoin_common`/`_server`/`_wallet` (`ref/ycash/src/Makefile.am:267,324,413`). | precision | Pure modules (`params`, `amount`, `payload`, `script`, `address`, `view`, `state`) go in `libbitcoin_common`; `db`, `index`, `policy` and `rpc/yellowback.cpp` in `libbitcoin_server`; `wallet`, `txbuilder` and `rpc/yellowbackwallet.cpp` in `libbitcoin_wallet`. (§4.2) |
| E9 | `yed_getnewaddress` did not add the key to the address book, unlike `getnewaddress`. | precision | It calls `SetAddressBook(keyID, "", "receive")` so the standard listing RPCs see the key. (§4.4) |
...
| F3 | The builder took YEC inputs from `AvailableCoins` with the default depth, which includes the wallet's own 0-conf outputs (B4's trusted rule) and outputs of the wallet's own *expired* transactions — `AvailableCoins` has no expiry filter and `RelayWalletTransaction` rebroadcasts any depth-0 transaction forever (`ref/ycash/src/wallet/wallet.cpp:4653-4668,5038-5052`). A Yellowback transaction chained on such an input can never confirm; for a MINT that means a wasted `evalHeight` window and a re-run. | hardening | Every input of every Yellowback transaction is confirmed: YED inputs from the index (already the rule) and YEC inputs via `AvailableCoins(..., nMinDepth = 1)`, the existing parameter (`wallet.cpp:5052`). No bespoke filtering. (§3.4, §4.5) |
...
| G2 | §6.0 generated roster keys 6–9 with `test_framework/key.py`, which binds OpenSSL through `ctypes` (`key.py:14-24`) — a host-library dependency that varies by platform and OpenSSL version. | precision | Keys 6–9 come from node 1's wallet (`getnewaddress` + `validateaddress` → `pubkey`), the node that never runs `-yellowback` and is never asked to co-sign. No Python crypto at all. (§6.0) |
| G3 | `rpc-tests.py -j4 yellowback_*` implied a glob. The runner accepts individual names only if they appear in its own lists (`qa/pull-tester/rpc-tests.py:213-218`) and does not glob. | precision | The Yellowback scripts are appended to `BASE_SCRIPTS` (already in the diff budget) and CI names them explicitly. (§6.0) |
| G4 | CI step 4 named `src/zcash-gtest --gtest_filter=Yellowback*`. The binary is `ycash-gtest` (`src/Makefile.gtest.include:6`) and the plan writes Boost tests only (§7). | precision | Step 4 is `src/test/test_bitcoin --run_test=yellowback_*`; the nightly `make check` covers gtest. (§6.0) |
| G5 | The regtest overrides table left the volatility windows at mainnet values (`BLOCKS_PER_DAY` = 1,152), so the 24-hour breach could never be exercised in a short test. `-yellowbacksupplycap` was introduced there but not in the configuration list. | precision | Regtest overrides for the two volatility windows (48 / 96); `-yellowbacksupplycap` (regtest only) added to §4.4. (§3.1, §4.4) |
| G6 | Node 1 (no `-yellowback`) must still share consensus parameters, and it has no index to compare. | precision | Node 1 gets the same six `-nuparams`; the state-hash assertion runs over the `-yellowback` nodes only. (§6.0, §7) |
...
and the workspace docs by the product name **Ycash Yellowback (YED)** under one rule: Yellowback
is the thing, YED is the unit (D15, AGENTS.md rule 6). No protocol rule changes, but three
identifier-level constants that carried the old name do:

| Was | Now | Where |
|---|---|---|
| payload magic `0x59 0x44` (`"YD"`) | `0x59 0x42` (`"YB"`) | §3.2 |
...
**Yellowback v1 is a Tier-0 build: no consensus change, no policy change, no new opcodes, no network
...
even fewer hooks: the Yellowback index subscribes to the existing `CValidationInterface::ChainTip`
signal instead of adding calls to `main.cpp`, and touches no file under `src/wallet/`.

The five load-bearing choices:

1. **Token representation.** A YED balance is an ordinary transparent output (P2PKH to the
   ...
   807-818`). Every Yellowback-aware node computes the *same* Yellowback state from the *same* chain, so
...
2. **Collateral vault.** Collateral is locked in a P2SH output whose redeem script is
   ...
   `ref/ycash/src/main.cpp:2931`). The "you must burn YED to get collateral back" rule is
...
5. **State.** A self-contained, rebuildable LevelDB index under `<datadir>/yellowback/` with per-block
...
the federation instead: (a) collateral cannot be released without burning the required YED, and
...
(`mapping.md` §5). Cost: a non-Yellowback-aware wallet that spends a YED-bearing output burns the
Yellowback (same property as Omni). Mitigation: the Yellowback wallet layer locks such coins
(`CWallet::LockCoin`, `ref/ycash/src/wallet/wallet.h:1369`) and a distinct address format (D10)
stops users from sending YED to plain `s1…` addresses by accident.
...
the collateral back *and keeps the YED they sold* — minting becomes a free loan that is never
...
**Decision: (b), Python, shipped in `ycash-dd/contrib/yellowback/`.** Price fetching needs an HTTPS
...
`yed_cosignredeem` RPC (vault spends). This keeps the C++ diff to the overlay and wallet.
...
tip by up to one second and every Yellowback RPC reports the index height it answers for
(`yed_getinfo.synced`); rebuilds read blocks from disk, so `-yellowback` refuses to start with `-prune`
...
(c) a dedicated `CDBWrapper` LevelDB under `<datadir>/yellowback/`.
...
per block so it can roll back in-process reorgs, and it can be wiped and rebuilt from the Yellowback
start height with `-reindex-yellowback`. Cost: a new directory and ~600 lines of DB code.

### D9. Wallet integration

Options: (a) extend `CWallet::CreateTransaction` to understand Yellowback · (b) build Yellowback
...
1,000 zat as every `z_*` operation (`ref/ycash/src/policy/fees.h:15`; `-yellowbackfee` overrides), so
...
`0x1F 0xE4` (mainnet, renders `ye…`), `0x20 0x07` (testnet, `yt…`), `0x20 0x02` (regtest, `yr…`),
decoding to an ordinary P2PKH destination.** Verified by exhaustive search over all payloads to
render those two leading characters. Implemented entirely in `src/yellowback/address.cpp`; no
...
(`digidollar.h:88`); maximum output $100,000, a Yellowback parameter set equal to DigiByte's maximum
...
**Decision:** an experimental-features flag `-yellowback` (requires `-experimentalfeatures`, pattern
of `ref/ycash/src/experimental_features.cpp`) plus a per-network **start height** and **genesis
anchor outpoint** in `src/yellowback/params.cpp`. No branch ID, no activation height in
`chainparams.cpp`. Nodes without the flag are unaffected in every way.

### D15. Naming and layout

**Decision:** the product is **Ycash Yellowback (YED)** — *Yellowback* names the system, *YED* is
the unit (same relationship as Bitcoin/BTC; the test: swappable for "dollars" → YED, swappable for
"the system" → Yellowback). In code: namespace `yellowback`, directory `src/yellowback/`, log
category `yellowback`, config prefix `-yellowback…`, types such as `YellowbackIndex`; unit-counting
...
dependency and no background operation**: `yed_redeem <vault>` builds and owner-signs the
transaction and returns its hex plus the vault's roster; `contrib/yellowback/yellowback-redeem` (Python,
`requests`, TLS verified) POSTs it to each operator's `/cosign` endpoint until `k` signatures are
present; `yed_submitredeem <hex>` re-verifies on the user's node — the transaction must be identical
to the one `yed_redeem` produced except for added signatures (SUB-1), and every input must pass
...
### D18. Semantics of invalid Yellowback transactions

**Decision (deterministic, applied by every node):** an invalid MINT registers a *void* vault
(collateral locked, no YED created; releasable at `lockHeight` with federation co-sign and zero
burn). An invalid TRANSFER burns all YED inputs. Any spend of a YED-bearing output that is
...
machine total (every chain has exactly one Yellowback state).

### D19. Enshrinement constraint

**Decision:** the validator is written as pure functions of `(transaction, YellowbackStateView,
height)` returning a typed verdict, with block-level `Apply`/`Undo` over an abstract key-value
view. This is the shape `ConnectBlock` would call in §9. No Yellowback code may read wall-clock time,
the mempool, wallet state, or node configuration inside the validator.

### D20. Testing

**Decision:** Boost unit tests for every pure function; `qa/rpc-tests/yellowback_*.py` regtest tests
...
co-signs redemptions with `yed_cosignredeem`, so the whole protocol is tested without the Python
...
`ycash-dd` · (c) add a Yellowback tab to **YecWallet** — the Qt 6 full-node GUI Ycash users already
run, which bundles `ycashd` and drives it over JSON-RPC — in a fork `yecwallet-dd`, driven
entirely by the `yed_*` RPCs · (d) RPC and `ycash-cli` only.

**Decision: (c), with `ycash-cli` as the first-class power-user and operator interface, and the
`yed_*` RPC surface frozen as the contract between the two forks.** (a) is impossible: Ycash has
...
|---|---|---|
| `src/init.cpp` | ≈ 40 | help text, `-prune` incompatibility check, regtest-only argument check, index open/sync/close, register validation interface, `-reindex-yellowback`, wallet-RPC registration under `ENABLE_WALLET` |
| `src/experimental_features.{h,cpp}` | ≈ 8 | `fExperimentalYellowback` |
| `src/rpc/register.h` | ≈ 6 | `RegisterYellowbackRPCCommands`, `RegisterYellowbackWalletRPCCommands` |
| `src/rpc/client.cpp` | ≈ 15 | numeric-argument conversions for `yed_*` |
...
Everything else is new files under `src/yellowback/`, `src/rpc/yellowback*.cpp`, `src/test/yellowback_*`,
`src/fuzzing/Yellowback*/`, `qa/rpc-tests/yellowback_*.py`, `contrib/yellowback/`, `doc/yellowback.md`, and one fork-local CI workflow
(`.github/workflows/yellowback-tests.yml`; Ycash's own CI runs only the `book.yml` docs job and no
functional tests, B6).
...
src/yellowback/
  ...
  address.{h,cpp}     Yellowback address encode/decode (D10) over CKeyID
  ...
  index.{h,cpp}       class YellowbackIndex : public CValidationInterface — ChainTip handler,
                      startup SyncToChain(), health flag, cs_yellowback lock, accessors for RPC
  ...
  txbuilder.{h,cpp}   (ENABLE_WALLET) BuildMint/BuildTransfer/BuildRedeem on
                      CreateNewContextualCMutableTransaction, flat YELLOWBACK_FEE, smallest-first inputs,
                      ...
                      (node context, no wallet) for yed_createpricetx
...
src/rpc/yellowback.cpp          node-context RPCs (work with pwalletMain == nullptr, C21)
src/rpc/yellowbackwallet.cpp    wallet-context RPCs
...
binary); `db`, `index`, `policy`, `rpc/yellowback.cpp` → `libbitcoin_server`; `wallet`, `txbuilder`,
`rpc/yellowbackwallet.cpp` → `libbitcoin_wallet` under `ENABLE_WALLET`.
contrib/yellowback/yellowback_fed.py      federation coordinator (§5)
contrib/yellowback/yellowback-redeem      user-side co-signature collector (§5)
```

The node has no networking code of its own for Yellowback: it never opens a connection, never runs a
background operation, and every Yellowback action is a synchronous RPC.

### 4.3 Hook points (with citations)

| Where | What | Why it is safe |
|---|---|---|
| `init.cpp` after wallet load (`CWallet::InitLoadWallet`, `ref/ycash/src/init.cpp:1782`), immediately before the notifier thread starts (`init.cpp:1831-1847`) | `if (fPruneMode) InitError`; `g_yellowback = new YellowbackIndex(...)`; `g_yellowback->SyncToChain()`; `RegisterValidationInterface(g_yellowback)` | Nothing else runs yet (block import is step 10, networking step 11), so the index is at `chainActive.Tip()` when notifications begin. Registered after the wallet, so `boost::signals2` delivers each signal to the wallet first; the index does not depend on that |
...
take `cs_yellowback` for the state update, collect the outpoints to lock or release, release
`cs_yellowback`, then take `pwalletMain->cs_wallet` for `LockCoin`/`UnlockCoin` (which assert
`cs_wallet`, `wallet.cpp:6265-6275`). `IsMine` needs only `cs_KeyStore`. RPC handlers take
`LOCK2(cs_main, pwalletMain->cs_wallet)` and then `cs_yellowback`, and never call back into the
wallet while holding `cs_yellowback`. With `-disablewallet` (`pwalletMain == nullptr`) the locking
...
and start empty (logged; also forced by `-reindex-yellowback`). Undo records older than `tip − 1,000`
...
Unhealthy state: every `yed_*` RPC except `yed_getinfo` returns `RPC_MISC_ERROR "yellowback index
unhealthy: <reason>; restart with -reindex-yellowback"`. The node itself never stops or fails a block
because of Yellowback.

### 4.4 RPC surface

All commands use Ycash's `fHelp` + `UniValue params` convention and are gated by
`fExperimentalYellowback` (pattern: `ref/ycash/src/rpc/atomicswap.cpp:1753`).

Node context (`src/rpc/yellowback.cpp`):

| Command | Purpose |
|---|---|
| `yed_getinfo` | enabled, `rpcversion`, `network` (`main`/`test`/`regtest`, so a GUI never infers it from `getinfo.testnet`, H3), network params, index height/hash, `synced` (index tip == `chainActive.Tip()`), health flag, anchor, roster index |
| `yed_getstatehash [height]` | SHA-256 over the canonical serialisation of every table (tests assert equality across nodes and across rebuilds) |
| `yed_getstats` | supply, collateral, active/void vaults, price, health, dcaBps, errBps, mintFrozen |
| `yed_getprice [height]` | `price(H)`, source height, age |
| `yed_getroster` | current roster pubkeys, k, n, script hex, P2SH address, previous roster |
| `yed_getvault <txid>` | vault record, including `requiredBurnAtClose` and `unbacked = burnedCents < requiredBurnAtClose` for CLOSED vaults (F1) |
| `yed_listvaults [status] [rosterIndex]` | paged list |
| `yed_gettxinfo <txid>` | the `TxLog` entry for a confirmed transaction (E1), or a dry-run for a mempool transaction, or `expired` for a wallet transaction past its `nExpiryHeight` that is in neither (F6); no `-txindex` needed |
| `yed_decodepayload <hex>` | payload decoder |
| `yed_validaterawtransaction <hex>` | dry-run §3.7 + §3.8 against the index tip, with the input view built from `pcoinsTip` + `CCoinsViewMemPool` under `cs_main` (C1); returns verdict, type, yedIn/yedOut/burn, fee, requiredBurn, reasons — used by co-signers and the coordinator |
| `yed_estimatecollateral <cents> <tier> [priceMicroUsd]` | `requiredZat` at current or given price and current DCA |
| `yed_gethistory <fromHeight> <toHeight>` | snapshots |
| `yed_createpricetx <priceMicroUsd\|"rotate"> [refill_txid:n] [newRosterScriptHex]` | unsigned PRICE transaction (or, with `"rotate"`, a ROTATION with no `OP_RETURN`, C9) spending the current anchor: `CreateNewContextualCMutableTransaction`, `YELLOWBACK_FEE`, anchor value from the index's `Anchor.nValue` cross-checked against the UTXO view (D4), anchor output to the current (or new) roster script, the optional refill absorbed whole into the anchor (must be confirmed, C6); returns hex plus the `prevtxs` array `signrawtransaction` needs when the previous anchor is in neither the chain nor the signer's mempool (C11). Ycash's `createrawtransaction` cannot emit an `OP_RETURN` (B1) |

Wallet context (`src/rpc/yellowbackwallet.cpp`, `ENABLE_WALLET`):

| Command | Purpose |
|---|---|
| `yed_getnewaddress` | fresh keypool key, added to the address book as `getnewaddress` does (E9), rendered as a Yellowback address |
...
| `yed_listpositions [status]` | my vaults with `canRedeem`, `requiredBurn`, `unlockHeight` |
| `yed_listtransactions [count] [skip]` | mint/send/receive/redeem history from `TxLog` (E1), filtered by `IsMine` over the assigned outputs' scripts and `HaveKey` over vault owner keys |
| `yed_lockcoins` | re-run coin locking (maintenance) |

Configuration: `-yellowback`; `-yellowbackstartheight`, `-yellowbackgenesisanchor` and
`-yellowbackgenesisroster` (regtest only, all three together, C2); `-reindex-yellowback`;
`-yellowbackfee=<zat>` (default and minimum `DEFAULT_FEE`, C16); `-yellowbackmintlag=<blocks>` (default
2, C4); `-yellowbacksupplycap=<cents>` (regtest only, default 0 = none, G5); `-debug=yellowback`. Co-signer URLs are configuration of the Python client, not of the node.

### 4.5 Wallet behaviour

- **Coin locking (three stages, B4).** (i) `yed_mint`/`yed_send`/`yed_redeem` call `LockCoin` on
  every YED output of the transaction they are about to commit **before** `CommitTransaction`,
  ...
  mint's token output, an output of a transaction that turned out non-Yellowback); outpoints that are
  ...
  `yed_send`/`yed_redeem` select Yellowback and vault inputs directly from the index — skipping
...
- **Ownership and keys.** `yed_mint` draws **one** fresh keypool key and uses it both as the vault
  ...
  unlocked (`wallet.cpp:6005-6022`), so `-keypool=1` needs no special case (C8); `yed_listpositions`
  ...
  (`wallet.cpp:4336-4348`); YED coins survive it and `yed_listtransactions` reads the index, not
...
- **Confirmed-only chaining.** The wallet never spends unconfirmed YED (DigiByte commit
...
- **Expiry.** A Yellowback transaction that expires unmined is dropped by the mempool at the next
  ...
  (`wallet.cpp:4653-4668`); the wallet does not mark it. For Yellowback: an expired MINT or TRANSFER
...
- **Wallet views.** `listunspent` and every coin-selecting RPC go through `AvailableCoins`, which
  skips locked coins (`wallet.cpp:5074`), so YED outputs never appear as spendable YEC;
...
- **Transaction building** (`txbuilder.cpp`, B14): start from
  ...
  the flat `YELLOWBACK_FEE` (a REDEEM has no YEC inputs, C10); change via
...
An operator runs a normal `ycashd` with `-experimentalfeatures -yellowback` on a **dedicated host**
...
`rawtransaction.cpp:966-974`, B2). The resulting address must equal `yed_getroster`. The operator
exposes `yed_cosignredeem` and `signrawtransaction` to the coordinator over local RPC only. A
...
(`controller.cpp:255-257`), which regtest reports as `false`; the Yellowback tab therefore takes its
network from `yed_getinfo.network` (H3). Sends go
through validation, a confirm dialog and `executeTransaction` (`src/sendtab.cpp:661-701`,
`controller.cpp:587-607`). The Yellowback work is a fork of this application, `yecwallet-dd`, on
...
Yellowback RPCs that already exist in §4.4; the table below is the functional specification for the
wallet application. Coin control is dropped for v1 (the node's builder selects YED inputs
...
|---|---|---|---|
| **Status banner** (every screen) | index syncing / synced / unhealthy; "Yellowback unavailable" with the reason and the fix (`-reindex-yellowback`) | `yed_getinfo` | shown whenever `synced` is false or `healthy` is false; every action button is disabled while it shows |
...
| File | Lines (est.) | What |
|---|---|---|
| `src/yellowbacktab.{cpp,h,ui}` | ≈ 600 | the Yellowback tab: a `QTabWidget` of the sub-pages below, added to `MainWindow`'s tab bar after Transactions |
| `src/yellowbackcontroller.{cpp,h}` | ≈ 500 | issues every `yed_*` call through the existing `Connection`; refresh hooked into `Controller::getInfoThenRefresh`'s "block changed" branch; pending redemptions watched on `txTimer` like `watchTxStatus` |
| `src/yellowbackmodels.{cpp,h}` | ≈ 400 | `QAbstractTableModel`s for positions and Yellowback transactions (pattern: `txtablemodel.h`, `balancestablemodel.h`) |
| `src/yellowbackmint.ui`, `yellowbacksend.ui`, `yellowbackreceive.ui`, `yellowbackredeem.ui`, `yellowbackpositions.ui`, `yellowbacktransactions.ui`, `yellowbackoverview.ui` | ≈ 1,200 (XML) | the sub-pages, laid out after DigiByte's `src/qt/digidollar*widget.cpp` behaviourally |
| `src/yellowbackredeemwizard.{cpp,h}` | ≈ 400 | the co-signature collector: HTTPS POSTs to operator `/cosign` endpoints via the app's existing `QNetworkAccessManager`, progress, retry of transient refusals, deadline countdown, abort |
| `src/connection.cpp` | ≈ 10 | `createZcashConf` also writes `experimentalfeatures=1` and `yellowback=1`; detection of an existing conf without them (`yed_getinfo` → "Method not found") and an offer to append |
...
expects it (`connection.cpp:348-364`), so a user who installs YecWallet-Yellowback gets a Yellowback-capable
node without knowing it; the wallet checks `yed_getinfo.rpcversion` at connect and refuses a node
it does not know.

**Cross-cutting rules.**

1. **The RPC surface is the contract.** `doc/yellowback-rpc.md` in `ycash-dd` documents every `yed_*`
   call, argument, result field and error code; `yed_getinfo.rpcversion` is bumped on any
...
(`yellowback-fed status`: price age, last round, peers, anchor value) and `yed_getinfo` are the
operator interface; the runbook (§5) is written for them.

**Light wallets.** A light client cannot compute the Yellowback index (it has no blocks), so the
light application is out of scope for v1. The path, if wanted later: a small indexer service that
runs `ycashd -yellowback` and exposes the read-only `yed_*` calls to light clients, alongside
...
The test is a Qt Test target that drives the Yellowback tab against that node: mint, send, `generate`
...
contract itself is covered by `yellowback_*.py` (§7), so a green node suite plus a green wallet test
...
## 5. Federation coordinator (`contrib/yellowback/`)

Python 3, single file per role, no daemons beyond `ycashd`. Configuration in `yellowback-fed.toml`
...
4. Each peer runs `yed_validaterawtransaction` and `decoderawtransaction` on its own node and
   ...
   one `OP_RETURN` with the stated price; fee = `YELLOWBACK_FEE`; and **no input other than the anchor is
...
coordinator asserts that the resulting P2SH address equals the one derived by `yed_getroster`.

**Redemption co-signing endpoint (`POST /cosign`, HTTPS, public):** body = owner-signed hex. The
coordinator calls `yed_cosignredeem` on its node, which applies §3.8 and signs only if every policy
...
**Redemption client (`contrib/yellowback/yellowback-redeem`):** `yed_redeem` on the user's node →
POST the hex to operators, one after another, from a published endpoint list (shipped with the
coordinator release and verifiable against `yed_getroster` key ids), each operator adding one
signature to the transaction it received, until `k` signatures are present → `yed_submitredeem` on
...
(≈ 45 minutes) of `yed_redeem`; past it, `yed_abortredeem` releases the locked inputs and the user
starts over (B24).

**Rotation (`yellowback-fed rotate`):** builds the ROTATION transaction (`yed_createpricetx rotate
...
`yed_listvaults rosterIndex=<old>` reports zero ACTIVE/VOID vaults.

**Operator runbook (in `doc/yellowback-federation.md`):** key generation inside the node and pubkey
...
`-datacarrier` left at default (C23), monitoring (`yed_getinfo`, `yed_getprice` age, coordinator
...
Yellowback reads only block heights — never wall-clock time (§3.10) — so time is compressed by
...
Standard topology for Yellowback functional tests (7 of the 8 available nodes):

| Node | Role | Flags |
|---|---|---|
| 0 | user wallet (minter, redeemer) | `-yellowback` |
| 1 | second user / plain relay node; source of roster keys 6–9 | **without** `-yellowback` but with the same six `-nuparams` — proves ordinary nodes relay and mine Yellowback transactions unchanged (§8.3); excluded from state-hash assertions (G6) |
| 2–6 | five federation operators, one roster key each, `addmultisigaddress` run on each (B2) | `-yellowback` |
...
node 1 never runs `-yellowback` and is never asked to co-sign — no Python cryptography, G2), puts
...
**3. The coordinator runs in the same tests.** `yellowback_federation.py` launches
`contrib/yellowback/yellowback_fed.py` as one subprocess per operator node, pointed at that node's RPC
...
testnet in Phase 7, not in CI). The `yellowback-redeem` client is driven the same way. Nothing in the
...
| Loop | Command | Time |
|---|---|---|
| Pure logic (payload, scripts, math, state machine) | `src/test/test_bitcoin --run_test=yellowback_*` | seconds |
| One flow end to end | `qa/rpc-tests/yellowback_lifecycle.py` | ≈ 1–2 min |
| Whole Yellowback suite | `qa/pull-tester/rpc-tests.py -j4 yellowback_index yellowback_lifecycle …` — names must be listed in the runner's `BASE_SCRIPTS`, which Phase 2+ do; it does not glob (`rpc-tests.py:213-218`, G3); it already parallelises (`:174`) | ≈ 10 min |
| Interactive multi-node playground | any `yellowback_*.py --noshutdown --nocleanup` (`test_framework.py:105-107`), then `ycash-cli -regtest -datadir=<tmpdir>/nodeN …` and `generate` by hand | as long as wanted |
...
`.github/workflows/yellowback-tests.yml`, plain GitHub Actions on `ubuntu-22.04`:
...
4. `src/test/test_bitcoin --run_test=yellowback_*` (the plan's unit tests are Boost; the whole
   `make check`, including `ycash-gtest`, runs in the nightly job, not on every push — G4).
5. `qa/pull-tester/rpc-tests.py -j4 <every yellowback_* script by name>` — the multi-node functional
...
passing in Phase 0, and `yellowback_reorg_stress.py`. Pull requests to `feature/digidollar` are
...
- [x] Replace `docs/spec/yellowback-adaptation-spec.md` with §3 of this plan (normative spec) and a
...
- [x] Create `ycash-dd/doc/yellowback.md` skeleton and `contrib/yellowback/README.md`.
...
Exit: `make status` clean; baseline test run recorded in `doc/yellowback.md`.
...
Exit: `src/test/test_bitcoin --run_test=yellowback_*` green; no file outside the new module touched
except the two Makefiles. **Done 2026-09-05** (`ycash-dd` commit "Yellowback Phase 1"); the math
header is `src/yellowback/math.h`, not `amount.h`, because a quoted `#include "amount.h"` from inside
`src/yellowback/` resolves to itself.
...
- [x] `qa/rpc-tests/test_framework/yellowback_util.py`: Ycash branch IDs (`YCASH 0x374d694f`,
      ...
      `chainparams.cpp:576-604`, C12) plus `-experimentalfeatures -yellowback` and the regtest genesis
      ...
      `miner.cpp:201-203`; every Yellowback test uses `setup_clean_chain = True`, B6);
      `assert_yed_synced(nodes)` = `sync_blocks(nodes)` then `yed_getinfo.synced` on each — the
...
- [x] `qa/rpc-tests/yellowback_index.py`: three regtest nodes started **without** `-yellowback`; test
      ...
      anchor still in the mempool without `prevtxs`, C11), checks `yed_getprice`, ages a price past
      ...
      `yed_getstatehash` on all nodes; restarts a node and asserts equality; restarts with
      `-reindex-yellowback` and asserts equality; restarts with `-reindex` and asserts equality;
      asserts that a node started with `-prune` and `-yellowback` fails to start.
...
- [x] `qa/rpc-tests/yellowback_lifecycle.py`: node 0 mints tier 0 at a mocked price, asserts vault
      ...
      the mint ACTIVE (C4); a block containing a YED output is disconnected with
      `invalidateblock` and `listlockunspent` still shows the output locked (C3); sends to node 1
      (`yed_send`), asserts conservation and `yed_getbalance` on both; a `yed_send` whose change
      would be under `MIN_OUTPUT` is refused (C20);
      node 1 spends a YED output with plain `sendtoaddress` after `lockunspent false` and the
      ...
      node 0 runs `yed_redeem`, passes the hex through `yed_cosignredeem` on federation nodes 1 and 2,
      `yed_submitredeem`, asserts vault CLOSED, supply and collateral decremented; a co-signer
...
- [ ] Copy review of every user-facing string against §8.1 (trust statement) — the wallet must
      never describe Yellowback as trustless or shielded; `git diff yecwallet-legacy...feature/digidollar`
      reviewed against the table in §4.7.

Exit: the QTest target passes in CI against a `yellowback-v1-rc1` node; a non-developer completes
...
|---|---|---|
| Unit | `src/test/yellowback_math_tests.cpp` | §3.6 formulas, overflow, table boundaries |
| Unit | `src/test/yellowback_payload_tests.cpp` | §3.2 codec, every malformed case |
| Unit | `src/test/yellowback_script_tests.cpp` | §3.3 scripts, `VerifyScript` success/failure, size and sigop limits, `IsStandardTx`/`AreInputsStandard` on every template (B5) |
| Unit | `src/test/yellowback_state_tests.cpp` | §3.7 rules on synthetic blocks; apply/undo identity; anchor custody |
| Unit | `src/test/yellowback_address_tests.cpp` | D10 |
| Functional | `qa/rpc-tests/yellowback_index.py` | index, prices, anchor at non-zero input index, reorg, restart, `-reindex`, `-reindex-yellowback`, `-prune` refusal |
| Functional | `qa/rpc-tests/yellowback_lifecycle.py` | mint/send/burn/redeem with RPC co-signing; 0-conf lock; evalHeight price drop |
| Functional | `qa/rpc-tests/yellowback_void_mint.py` | D18 outcomes |
| Functional | `qa/rpc-tests/yellowback_wallet_restore.py` | restore, encryption |
| Functional | `qa/rpc-tests/yellowback_federation.py` | Python coordinator, HTTP co-sign, rotation |
| Functional | `qa/rpc-tests/yellowback_protection.py` | DCA, ERR, volatility |
| Functional | `qa/rpc-tests/yellowback_reorg_stress.py` | randomized reorgs, state-hash equality |
| Fuzz | `src/fuzzing/YellowbackPayload/`, `src/fuzzing/YellowbackScript/` | parser robustness (C14) |
...
Every functional test starts its nodes with `yellowback_node_args()` (six upgrades at height 1,
`setup_clean_chain = True`, C12), uses the topology of §6.0 (node 1 without `-yellowback`; 2-of-3 on
nodes 2–4 for lifecycle tests, 5-of-9 with four test-held keys for `yellowback_federation.py`),
relies on `sync_all()` for index synchrony (C13) and asserts `yed_getstatehash` equal on all
`-yellowback` nodes at the end (G6). Unit tests that call `IsStandardTx`, `AreInputsStandard` or
...
(`policy.cpp:58-68`, C19, D6). The regtest federation uses fixed test keys (`SHA256("yellowback_regtest_oracle_N")`,
...
- **Therefore:** a colluding quorum of k operators can release collateral without a burn or publish
  ...
  recovers. Nothing the federation does can create YED out of nothing, move a user's YED, or
...
|---|---|---|
| k operators collude on price | under-collateralised mints (same as DigiDollar's 7-of-35) | roster diversity, exchange-median policy, public price history (`yed_gethistory`), rotation |
| k operators + owner collude on release without burn | unbacked YED equal to that vault's mint | visible on chain (`unbacked` flag: `burnedCents < requiredBurnAtClose`, F1); health falls; rotation of the colluding members |
...
| Wallet spends Yellowback as YEC | that Yellowback is burned | coin locking; Yellowback address format |
...
| `sendtoaddress` right after `yed_mint`/`yed_send` | none: outputs locked before commit (B4) | three-stage locking; locks survive disconnects (C3) |
...
| Relay node runs `-datacarrier=0` | that node does not relay Yellowback transactions; others do | runbook (C23) |
| Miner malleates a Yellowback transaction's signatures (txid changes; no SegWit) | index unaffected (keyed on confirmed outpoints); the wallet's copy shows as conflicted, as for any Ycash transaction today | positions and balances derived by key ownership, never by remembered txid (E7) |
...
| DoS via `yed_validaterawtransaction` / `/cosign` | CPU on the node/coordinator | bounded parsing, rate limits, RPC auth |
...
whether or not `-yellowback` is enabled. A crash bug in `src/yellowback/` must be caught at the
...
5. All `yed_*` RPCs refuse when the flag is off or the index is unhealthy.
6. Coin locking covers every mine-owned token and vault outpoint after restart.
7. `yed_cosignredeem` never signs when §3.8 fails; test proves it.
8. No new build dependencies; `configure.ac` untouched; `grep -rn "evhttp\|socket\|connect(" src/yellowback src/rpc/yellowback*.cpp` returns nothing.
...
10. `-yellowback -prune` fails at init; `-reindex` rebuilds to the same `yed_getstatehash`.
11. `IsStandardTx` and `AreInputsStandard` pass in unit tests for every template (regtest does not
    run them).
12. A `sendtoaddress` issued immediately after `yed_mint`/`yed_send` cannot select a YED output.
13. Every `MINT-4`/`MINT-5` input is read from `Snapshots[evalHeight]`; nothing in the mint rules
    reads `price(H)` or `health(H)`.
14. `yed_createpricetx` output round-trips through `signrawtransaction` on a node that ran
    `addmultisigaddress`, and fails (`complete: false`) on one that did not.
15. `yed_cosignredeem` computes the sighash from the index's vault record only; a test with a
...
18. `-yellowbackfee` below `DEFAULT_FEE` is rejected at init (C16); `-yellowbackstartheight` and the
    two genesis arguments are rejected outside regtest and must appear together (C2).
19. `yed_validaterawtransaction`/`yed_cosignredeem` refuse a transaction with an unknown or spent
    input **before** touching `AreInputsStandard` or the fee (D1); a unit test proves no assert.
20. The index object is unregistered and stopped under `cs_yellowback` in `Shutdown()` and never
...
`UPGRADE_YELLOWBACK` added before `UPGRADE_ZFUTURE` in `Consensus::UpgradeIndex` with a fresh branch
...
5. The Yellowback state becomes consensus state: `db.cpp` moves into the chainstate flush path (same
...
| DigiDollar (v9.26.5) | Yellowback v1 | Reason |
|---|---|---|
...
|---|---|---|
| Federation membership (9 operators, 5 threshold) | — | Ycash community; recorded in `doc/yellowback-federation.md` |
...
- **Token output** — a transparent output assigned YED cents by a payload.
- **Void mint** — a MINT whose payload parsed but whose rules failed; collateral is locked, no
  YED exists.
- **Index** — the node-local, rebuildable Yellowback state database.
