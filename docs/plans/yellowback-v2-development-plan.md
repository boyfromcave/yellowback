...
**Execution status (2026-09-10, updated by the implementing session):** Phase 0 complete (both forks; node `be116eeda`+merges, wallet `70263cb`, workspace `a032708`). Phase 1 complete (merged as `77fa95b0b`). The quote-agent half of Phase 7 and the Python model of §3 (`yellowback_model.py`, Phase 6 bullet) are merged. In progress: Phase 2 (state machine), the v2 `yellowback_util.py` (§6.0 item 4), `doc/yellowback-rpc.md` v2 (Phase 3/6 gate). Not started: Phases 3, 4, 5, 6 (rest), 7 (rest), 7b, 8. Phases 9–10 need real pools and cannot be executed on one machine. Per-checkbox state is in §6; deviations found while building are in `docs/mapping.md` §13.x.
