...
**Execution status (2026-09-10, updated by the implementing session):** Phase 0 complete (both forks; node `be116eeda`+merges, wallet `70263cb`, workspace `a032708`). Phase 1 complete (merged as `77fa95b0b`). Phase 2 complete (`f86580c2a`, with the golden state hash matching the Python model). The quote-agent half of Phase 7, the Python model of §3 (`yellowback_model.py`, Phase 6 bullet), the v2 `yellowback_util.py` (§6.0 item 4), and `doc/yellowback-rpc.md` v2 are merged. Phase 3 is in progress in the node working tree: synchronous hooks, node RPCs and tag emission build; the six-node framework and the v2 index smoke test pass. Not started: Phases 4, 5, 6 (wallet builders/RPCs), 7 (rest), 7b (wallet UI actions), 8. The legacy federation devnet and functional scripts are not v2-compatible yet. Phases 9–10 need real pools and cannot be executed on one machine. Per-checkbox state is in §6; deviations found while building are in `docs/mapping.md` §13.x.
