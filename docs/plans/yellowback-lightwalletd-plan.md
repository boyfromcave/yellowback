...
- [ ] `common/yellowback.go`: probe, `callYed`, allow-list, error mapping.
- [ ] `frontend/yellowback.go`: the handlers; `frontend/yellowback_addr.go` (D-L-4).
- [ ] `cmd/server/main.go`: flag, probe, register (≤ 30 lines).
- [ ] F-1: `SendTransaction`'s `errParts[1]` guard (§8) — the only baseline fix.
- [ ] `frontend/yellowback_test.go` (§6.2) incl. the allow-list completeness case.
- [ ] `docs/yellowback.md` first version.
**Acceptance:** offline suite green; `git diff --stat lightwalletd-legacy` shows `service.proto`,
`compact_formats.proto`, `parser/`, `common/common.go`, `common/cache.go` at **0**; devnet: every
method answers on node 0, baseline byte-equality gate green with the flag on and off.

### Phase N1 — `yed_listtokens` in the node (≈ 2 days; ycash-dd, RPC + docs + tests only)
- [ ] `src/rpc/yellowback.cpp`: the RPC of §4.4, registered node-context.
- [ ] `doc/yellowback-rpc.md` row; contract entry with example values; `make spec` regenerates
      the copies (now three).
- [ ] `yellowback_rpc_contract.py` case; `yellowback_index.py` case vs `yed_listunspent` and the
      model (`yellowback_model.py` gains `tokens_by_script()` — a filter over its `Tokens`).
- [ ] Frozen-file proof: `git diff --stat feature/yellowback-sf -- <frozen list>` empty; `main.cpp`
      unchanged.
**Acceptance:** functional tests green unarmed and `--armed`; contract check green in all three
forks.
